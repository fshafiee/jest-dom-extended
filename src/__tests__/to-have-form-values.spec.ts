...
function renderCheckboxes(name, label, options, value: any[]) {
  return `
    <fieldset>
      <legend>${label}</legend>
      ${renderList(
...
function renderSelectSingle(name, label, options, value?: string) {
  return renderSelect(
    name,
    label,
    options,
...
function renderSelectMultiple(name, label, options, value: any[]) {
