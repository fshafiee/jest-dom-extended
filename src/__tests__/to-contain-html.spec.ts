...
describe('.toContainHTML', () => {
  test('handles positive and negative cases', () => {
    ...
    expect(() =>
      expect(nonExistantElement).not.toContainHTML(
        nonExistantElement as unknown as string,
      ),
    ).toThrowError()
    expect(() =>
      expect(stringChildElement).not.toContainHTML(
        fakeElement as unknown as string,
      ),
    ).toThrowError()
    expect(() =>
    ...
    expect(() =>
      expect(child).toContainHTML(nonExistantElement as unknown as string),
    ).toThrowError()
    expect(() =>
      expect(parent).toContainHTML(nonExistantElement as unknown as string),
    ).toThrowError()
    expect(() =>
      expect(grandparent).toContainHTML(
        nonExistantElement as unknown as string,
      ),
    ).toThrowError()
    expect(() =>
